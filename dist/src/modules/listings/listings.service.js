...
let ListingsService = class ListingsService {
  ...
  constructor(prisma, listingAttributesService) {
    this.prisma = prisma;
  ...
  async create(createListingDto, sellerId) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (attributes && attributes.length > 0) {
        await this.listingAttributesService.bulkUpsert(listing.id, {
          attributes,
        });
      }
      return listing;
  ...
  async findAll(searchDto) {
    ...
    const listingDtos = await Promise.all(
      listings.map((listing) => this.mapToResponseDto(listing)),
    );
    const meta = new paginated_listings_dto_1.PaginationMetaDto(
      page || 1,
      limit || 20,
      total,
    );
  ...
  async getRelatedListings(listingId, limit = 4) {
    ...
    return await Promise.all(
      relatedListings.map((listing) => this.mapToResponseDto(listing)),
    );
  }
  ...
  async publish(id, userId) {
    ...
    if (listing.sellerId !== userId) {
      throw new common_1.ForbiddenException(
        'You can only publish your own listings',
      );
    }
  ...
  async extendExpiration(id, userId, days = 30) {
    ...
    if (listing.sellerId !== userId) {
      throw new common_1.ForbiddenException(
        'You can only extend your own listings',
      );
    }
  ...
  async mapToResponseDto(listing) {
    ...
    if (listing.attributes) {
      ...
      attributes = await this.listingAttributesService.findByListing(
        listing.id,
      );
    }
    return new listing_response_dto_1.ListingResponseDto({
      ...
      category: listing.category
        ? new category_response_dto_1.CategoryResponseDto(listing.category)
        : null,
      attributes,
    });
  }
